var https = require('https'),
    ...
    var username =  's-nero',
        ...
        getChartResult = function(callback){
            ...
            var today = new Date();
            var dayOfSprint = getDayOfSprint(getSprintStartDate(sprint), today);
            ...
            getSprintTasks(sprint, function(data){
                ...
                dots = [];
                dots.push('Tasks Remaining');

                var totalDoneCount = 0;
                for(i = 0; i < dayOfSprint ; i++){

                    // need to exclude weekends
                    thisDay = today.addDays(-(dayOfSprint-i));
                    doneCountThisDay = getDoneCountByThisDay(thisDay);
                    totalDoneCount += doneCountThisDay;

                    dots.push(totalIssue - totalDoneCount); 
                }

                function getDoneCountByThisDay(date){
                    var dayWithoutTime = dateToString(date);
